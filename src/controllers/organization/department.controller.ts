...
import { BranchModel, DepartmentModel, ShiftModel } from "../../infrastructure/database/models";
...
export const updateDepartment = async (
    ...
    try {
        ...
        if (assignments !== undefined)
            department.assignments = assignments;
...
export const getBranchShiftOptions = async (
    ...
    try {
        ...
        return res.status(200).json(
            ApiResponse.success(
                data,
                "Branch shift options fetched successfully"
            )
        );
    } catch (error) {
        next(error);
